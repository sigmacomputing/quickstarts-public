 ...
 * Updates a bookmark's exploreKey in Sigma API
 */
router.put("/update", async (req, res) => {
  try {
    const { workbookUrlId, bookmarkId, exploreKey } = req.body;

    if (!workbookUrlId || !bookmarkId || !exploreKey) {
      return res.status(400).json({ message: "Missing required fields: workbookUrlId, bookmarkId, exploreKey" });
    }

    if (DEBUG) console.log("Updating bookmark exploreKey:", { workbookUrlId, bookmarkId, exploreKey });

    // Step 1: Resolve workbook ID
    const workbook = await resolveWorkbookId(workbookUrlId);
    const workbookFullId = workbook.id;
    ...
    // Step 3: Update the bookmark in Sigma API
    const updateUrl = `${process.env.BASE_URL}/workbooks/${workbookFullId}/bookmarks/${bookmarkId}`;
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update bookmark: ${response.status} ${errText}`);
    }

    if (DEBUG) console.log(`Successfully updated bookmark ${bookmarkId} exploreKey to ${exploreKey}`);

    res.status(200).json({ 
      success: true, 
      message: "Bookmark exploreKey updated successfully",
      exploreKey: exploreKey
    });

  } catch (err) {
    if (DEBUG) console.error("Bookmark update error:", err.message);
 ...
 * Deletes a single bookmark by ID from Sigma API
 */
router.delete("/delete", async (req, res) => {
  try {
    const { workbookUrlId, bookmarkId } = req.body;

    if (!workbookUrlId || !bookmarkId) {
      return res.status(400).json({ message: "Missing required fields: workbookUrlId, bookmarkId" });
    }

    if (DEBUG) console.log("Deleting bookmark:", { workbookUrlId, bookmarkId });
    ...
    // Step 3: Delete the bookmark from Sigma API
    const deleteUrl = `${process.env.BASE_URL}/workbooks/${workbookFullId}/bookmarks/${bookmarkId}`;
    ...
    if (!response.ok) {
      const errText = await response.text();
      throw new Error(`Failed to delete bookmark: ${response.status} ${errText}`);
    }

    if (DEBUG) console.log(`Successfully deleted bookmark ${bookmarkId}`);
    ...
    if (DEBUG) console.error("Bookmark deletion error:", err.message);

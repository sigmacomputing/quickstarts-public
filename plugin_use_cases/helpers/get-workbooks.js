...
/**
 * findWorkbookByName - Finds a specific workbook by name within a team
 * @param {string} workbookName - Name of the workbook to find
 * @param {string} teamName - Team/workspace to search in
 * @returns {Promise<Object|null>} Workbook object or null if not found
 */
async function findWorkbookByName(workbookName, teamName) {
  const workbooks = await getWorkbooksByTeam(teamName);
  return workbooks.find(w => w.name === workbookName) || null;
}

module.exports = { getWorkbooksByTeam, findWorkbookByName };
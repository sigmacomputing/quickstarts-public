...
app.post("/embed-url", async (req, res) => {
  ...
  let resolvedEmail = config.email;
  ...
  if (!resolvedId && user) {
    ...
    if (config.memberIds[role]) {
      ...
      if (role === "view") resolvedEmail = config.viewEmail;
      if (role === "build") resolvedEmail = config.buildEmail;
    } else {
  ...
  try {
    ...
    const payload = {
      sub: resolvedEmail, // ✅ use EMAIL as sub
      path: embedPath,
